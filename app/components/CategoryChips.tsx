  ...
  return (
    <section className="min-w-0">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-xl font-black text-[#102b52]">
          Browse by Category
        </h3>

        <Link
          href="/search"
          className="text-sm font-bold text-[#149EAF]"
        >
          See all →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2.5 sm:grid-cols-4">
        ...
        {homeCategories.map((category) => (
          <Link
            ...
            className={`flex h-[58px] items-center justify-center gap-2 rounded-2xl ${category.bg} px-2 transition hover:-translate-y-0.5 hover:shadow-sm active:scale-[0.98]`}
          >
            <span className={category.color}>
              <CategoryIcon type={category.icon} />
            </span>

            <span className="text-[12px] font-bold leading-tight text-[#102b52]">
              {category.name}
            </span>
          </Link>
        ))}
